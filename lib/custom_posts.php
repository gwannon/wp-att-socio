...
function wp_att_socio_show_custom_fields() { //Show box
  ...
  $fields = wp_att_socio_get_custom_fields ($type); ?>
		<div>
                ...
                <div>
                  <?php foreach($datos['fields'] as $key => $subfields) { ?>
                    <?php if($subfields['tipo'] == 'image') { ?>
                      <input type="hidden" class="_<?php echo $type; ?>_<?php echo $field; ?>_<?php echo $key; ?>" id="input_<?php echo $type; ?>_<?php echo $field; ?>_<?php echo $key; ?>_<?php echo $i; ?>" style="width: 80%;" name="_<?php echo $type; ?>_<?php echo $field; ?>[<?php echo $i; ?>][<?php echo $key; ?>]" value="<?php echo $rest[$i][$key]; ?>" placeholder="<?php echo $subfields['titulo']; ?>" />
...
function wp_att_socio_save_custom_fields( $post_id ) { //Save changes
	global $wpdb;
		...
		foreach ($fields as $field => $datos) {
		  ...
		  if ($datos['tipo'] == 'repeater') { //Limpiamos vacios
				if(isset($_POST[$label])) {
				  $temp = wp_att_socio_array_remove_empty($_POST[$label]);
				  unset($_POST[$label]);
				  $_POST[$label] = array();
